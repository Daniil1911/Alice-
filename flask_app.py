...
def text_ps(res, req):
    ...
    if current_dialog == "txt_pesni":
        if req['request']['original_utterance'].lower() in ['altwall.net']:
            res['response']['card'] = {}
            res['response']['card']['title'] = "Одну секунду.."
            res['response']['card']['url'] = "https://altwall.net/texts.php"
            res['response']['card']['hide'] = "True"
            return
        if req['request']['original_utterance'].lower() in ['хватит']:
            current_dialog = "start"
            current_status = "start"
...
def photo_f(res, req):
    ...
    if current_dialog == "ph":
        if req['request']['original_utterance'].lower() in ['чернозём 2018']:
            res['response']['card'] = {}
            res['response']['card']['title'] = "Одну секунду.."
            res['response']['card']['url'] = "https://yandex.ru/images/search?text=чернозём%202018"
            res['response']['card']['hide'] = "True"
            return
